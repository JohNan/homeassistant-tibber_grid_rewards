...
class TibberPublicAPI:
    ...
    async def get_homes(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self._client.post(PUBLIC_API_URL, headers=self.headers, json={"query": query})
    ...
    async def get_price_info(self, home_id: str) -> Dict[str, Any] | None:
        ...
        try:
            response = await self._client.post(PUBLIC_API_URL, headers=self.headers, json=payload)
            response.raise_for_status()
            _LOGGER.debug("Successfully fetched price info from public API.")
            data = response.json()
            return data.get("data", {}).get("viewer", {}).get("home", {}).get("currentSubscription", {}).get("priceInfo")
        except httpx.HTTPStatusError as e:
            if e.response.status_code in (401, 403):
                _LOGGER.error("Authentication failed with public API.")
                raise TibberPublicAuthError from e
            ...
            _LOGGER.error("An unexpected error occurred while fetching price info: %s", e)
            return None